...
int parse_query_string(char* query_string, int* pairs_num, KeyValuePair* pairs) {
    ...
    // if (pairs == NULL) {
    //     printf("Failed to allocate memory for KeyValuePair\n");
    //     return NULL;
    // }
    ...
    for (i = 0; i < pairs_num; i++) {
        if (pairs[i].key != NULL && pairs[i].value != NULL) {
            ...
            if (strcasecmp(pairs[i].key, "cover") == 0) {
                ...
                // } else {
                //     printf("server_url read from push.conf is wrong!\n");
                // }
                ...
                //         printf("Set server_url to TEST_SERVER_URL\n");
                ...
                //         printf("Set server_url to OFFI_SERVER_URL\n");
...
//     device_info.used_ram_perct); strcat(device_info_content, temp); temp[0] = '\0';
